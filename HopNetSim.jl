...
function SimManyK(WeigthMatrix,IniStates, K)
...
manyEigVec[:,:,:,1], manyMat[:,:,:,1] = SimulationOne(WeigthMatrix, IniStates, K[1])
    for k in 2:length(K)
    manyEigVec[:,:,:,k], manyMat[:,:,:,k] = SimulationOne(WeigthMatrix, IniStates, K[k]);
    end
manyEigVec, manyMat
end
